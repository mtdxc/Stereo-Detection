...
void stereo_match(int, void*)
{
	...
	static Ptr<cv::StereoSGBM> sgbm = cv::StereoSGBM::create(mindisparity, ndisparities, blockSize);
	/*
...
/*****立体匹配*****/
void stereo_match_gpu(int, void*)
{
	int blockSize = 8, mindisparity = 1, ndisparities = 64, img_channels = 3;
	static Ptr<cv::cuda::StereoSGM> sgbm = cv::cuda::createStereoSGM(mindisparity, ndisparities, blockSize);
	/*
	bm->setBlockSize(2 * blockSize + 5);     //SAD窗口大小，5~21之间为宜
	bm->setROI1(validROIL);
	bm->setROI2(validROIR);
	...
	*/
	...
	Mat disp, disp8;
	cuda::GpuMat gImageL, gImageR, gDisp;
	gImageL.upload(rectifyImageL);
	gImageR.upload(rectifyImageR);
	sgbm->compute(gImageL, gImageR, gDisp);//输入图像必须为灰度图
	gDisp.download(disp);
	disp8 = Mat(disp.rows, disp.cols, CV_8UC1);
	normalize(disp, disp8, 0, 255, NORM_MINMAX, CV_8UC1);
	reprojectImageTo3D(disp, xyz, Q, true); 
...
static void onMouse(int event, int x, int y, int, void*)
{
	...
	case EVENT_LBUTTONDOWN:   //鼠标左按钮按下的事件
	{
		...
		float d = point3[0] * point3[0] + point3[1] * point3[1] + point3[2] * point3[2];
		d = sqrt(d);   //mm
	   // cout << "距离是:" << d << "mm" << endl;
		cout << "距离是:" << d / 10.0 << "cm" << endl;
		break;
	}
	...
	*/
	...
	while (cap.read(img)) {
		...
		*/
		...
		if (gpu)
			stereo_match_gpu(0, 0); // > 60fps
		else
			stereo_match(0, 0); // < 10fps

		//完成计时，计算帧率
		tm.stop();
		// 输出帧率
		char line[32];
		sprintf(line, "fps=%.2f", tm.getFPS());
		putText(canvas, line, Point(0, 40), FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2);
